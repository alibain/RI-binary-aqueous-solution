...
# 2 inorganic ions or 1 organic solute. Each speciece entered as ['name', mass fraction]
#input=[['Na',0.08],['Cl',0.12]]
input=[['NH4',0.0546],['SO4',0.14539]]
#input=[['malonic acid',0.2]]
...
# Check mass fractions and ion ratios before going on to the RI calculation
wB=sum(wBi)
wA=1-wB
if wA >= 1:
    sys.exit('!!sum of mass fractions of solutes must be less than 1!!')

mws=iondata.loc[:,"MW"]
charges=iondata.loc[:,"Charge"]
dv=[]
for i in range(len(solute)):
    dvi=(wBi[i]/(mws[i]*charges[i]))*sum(mws)
    dv.append(dvi)
...
if len(solute) <=0:
    ...
    print(dv1)
    if dv1 != 0:
        sys.exit("!!charges are not balanced!!")
